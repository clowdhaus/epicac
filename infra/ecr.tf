...
resource "local_file" "build_script" {
  ...
  aws ecr get-login-password --region ${local.region} | finch login --username AWS --password-stdin ${data.aws_caller_identity.current.account_id}.dkr.ecr.${local.region}.amazonaws.com

  TAG=epicac-$(date +%Y%m%d_%H%M%S)

  finch build --platform amd64 -t ${module.ecr.repository_url}:$${TAG} .
  finch push ${module.ecr.repository_url}:$${TAG}
